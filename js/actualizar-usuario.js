...
form.addEventListener('submit', (e) => {
    ...
    // Validaciones
    if (!tipoDocumento.value || tipoDocumento.value === "default") return showAlert("Debe seleccionar un tipo de documento.", true);
    if (!nombre.value) return showAlert("El nombre es obligatorio.", true);
    if (!numeroDocumento.value) return showAlert("El número de documento es obligatorio.", true);
    if (!telefono.value) return showAlert("El teléfono es obligatorio.", true);
    if (!correo.value) return showAlert("El correo electrónico es obligatorio.", true);
    if (!confirmarCorreo.value) return showAlert("Debe confirmar el correo electrónico.", true);
    if (!rol.value || rol.value === "default") return showAlert("Debe seleccionar un rol.", true);

    // Valido que los correos coincidan
    if (correo.value !== confirmarCorreo.value) return showAlert("Los correos electrónicos no coinciden.", true);
